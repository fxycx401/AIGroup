...
# os.environ["TORCH_DISABLE_CUDA"] = "1" #强制禁用 CUDA
os.environ['CUDA_LAUNCH_BLOCKING'] = '1'
...
if DEVICE.type == 'cpu': # cpu, 本机
    ...
    """
    # 修改 num_workers 为较小值（如4）
    data_loader = DataLoader(dataset, batch_size=params['batch_size'], num_workers=4,
                             pin_memory=True,prefetch_factor=params['prefetch_factor'])
    """
...
if __name__=="__main__":
    ...
    output_file.close()
    os.makedirs('model', exist_ok=True)# 保存模型前，确保 model 目录存在
    torch.save(model, f=f'model/{time_now}.pt')
    os.makedirs('chart_data', exist_ok=True)# 确保 chart_data 目录存在
