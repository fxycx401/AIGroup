...
class GIKT(Module):
    ...
    def __init__(self, num_question, num_skill, q_neighbors, s_neighbors, qs_table, device,agg_hops=3, emb_dim=100,
    ...
    def forward(self, question, response, mask):
        ...
        h1_pre = torch.nn.init.xavier_uniform_(torch.zeros(self.emb_dim, device=current_device).repeat(batch_size, 1))
        h2_pre = torch.nn.init.xavier_uniform_(torch.zeros(self.emb_dim, device=current_device).repeat(batch_size, 1))
        state_history = torch.zeros(batch_size, seq_len, self.emb_dim, device=self.device)
        ...
        for t in range(seq_len - 1): # 第t时刻
            ...
            emb_question_t = torch.zeros(batch_size, self.emb_dim, device=self.device)
            ...
            qs_concat = torch.zeros(batch_size, max_num_skill + 1, self.emb_dim).to(self.device)
            ...
            if self.hard_recap:
                ...
                current_history_state = torch.zeros(batch_size, max_num_states, self.emb_dim).to(self.device)
                # 当前状态
